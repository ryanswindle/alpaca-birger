"""
ASCOM Alpaca Birger EF-232 Focuser Server – Main FastAPI Application

Entrypoint that:
  - Creates the FastAPI application
  - Configures logging (loguru + stdlib intercept)
  - Instantiates FocuserDevice instances from config.yaml
  - Starts the Alpaca discovery responder (UDP 32227)
  - Includes management, setup, and focuser routers
...
# Device registry: device_number → FocuserDevice
...
async def lifespan(app: FastAPI):
    ...
    for device_config in config.devices:
        dev = FocuserDevice(device_config)
...
app = FastAPI(
    title="ASCOM Alpaca Birger EF-232 Focuser Server",
    description="ASCOM Alpaca API for the Birger EF-232 Canon EF lens controller",
    version="1.0.0",
    lifespan=lifespan,
)
