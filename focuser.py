...
async def tempcomp_put(devnum: int, TempComp: Annotated[str, Form()], params: AlpacaPutParams = Depends()):
    return MethodResponse.create(
        client_transaction_id=params.client_transaction_id,
...
async def halt(devnum: int, params: AlpacaPutParams = Depends()):
    device = get_device(devnum)
    if not device.connected:
        return MethodResponse.create(
            client_transaction_id=params.client_transaction_id,
...
async def move(devnum: int, Position: Annotated[str, Form()], params: AlpacaPutParams = Depends()):
    device = get_device(devnum)
    if not device.connected:
        return MethodResponse.create(
            client_transaction_id=params.client_transaction_id,
